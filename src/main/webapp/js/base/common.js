        ...
        ajaxGet: function (param) {
            ...
            $.ajax({
                ...
                success: function (data) { //客户端jquery预先定义好的callback函数,成功获取跨域服务器上的json数据后,会动态执行这个callback函数
                    if (data.code == -2) { //未登录-2，，
                    ...
                    } else if (data.code == -1 || data.code == 400) { //参数不合法-1
                        console.log("没有权限访问，返回登录界面，msg:" + data.msg);
                        /*$.cookie('is_login', '', { path: "/", expires: 7 });
                        $.cookie('u_id', '', { path: "/", expires: 7 });
                        $.cookie('u_name', '', { path: "/", expires: 7 });
                    	...
                    	// 临时的处理方式：还没想到更好的方式：当用户登录失效后，执行ajax操作后重定向到login.html
                    	console.log("不识别的code:" + data.code);
                    }
                },
                error: function () {
                    console.log("ajaxGet is error");
                    alert("ajaxGet is error!");
                    //location.href = "/html/login.html";
                },
                complete: function (XMLHttpRequest,textStatus) {
                    ...
                    // 参考1：ajax异步请求302
                    // https://www.cnblogs.com/52XF/p/post302.html
                    // 参考2：ajax302请求重定向解决跳转失败，包括shiro session会话重定向问题 
                    // https://blog.csdn.net/visket2008/article/details/81000632
